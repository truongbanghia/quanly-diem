<?php
    ...
    if (isset($_POST['sbm'])) {
        ...
        $sql_dayHoc = "SELECT * FROM dayhoc WHERE MaMonHoc = '$id_mon'";
        $query_dayHoc = mysqli_query($conn,$sql_dayHoc);

        if (mysqli_num_rows($query_dayHoc) != "") {
            
            $sql_update = "UPDATE dayhoc SET MaMonHoc = '$id_mon', MaGV = '$maGV', MaLopHoc = '$id_class', MaHocKy = '$maHK', MoTaPhanCong = '$details' WHERE MaMonHoc = '$id_mon'";
